...
Evaluate:
    python pick_cube_ppo.py --evaluate --checkpoint runs/<run>/final_ckpt.pt
'''
...
class Args:
    ...
    obs_mode: str = "state"
    # pd_ee_delta_pose: 6D EE delta + 1D gripper — gives full control over
    # both approach orientation and finger closure
    control_mode: str = "pd_ee_delta_pose"
    num_envs: int = 512
    num_eval_envs: int = 8
    reconfiguration_freq: Optional[int] = None
    eval_reconfiguration_freq: Optional[int] = 1
    num_steps: int = 100            # matches PickSkillEnv max_episode_steps=100
...
class Agent(nn.Module):
    def __init__(self, envs):
        ...
        # Pick obs: qpos(9) + qvel(9) + is_grasped(1) + tcp_pose(7) + obj_pose(7)
        #           + tcp_to_obj_pos(3) ≈ 36 dims; 3×256 is adequate capacity.
