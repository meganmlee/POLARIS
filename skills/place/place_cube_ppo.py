...
Evaluate:
    python place_cube_ppo.py --evaluate --checkpoint runs/<run>/final_ckpt.pt
'''
...
class Args:
    ...
    obs_mode: str = "state"
    # pd_ee_delta_pose: 6D EE delta + 1D gripper — full control over descent
    # angle and finger opening for precise release
    control_mode: str = "pd_ee_delta_pose"
    num_envs: int = 512
    num_eval_envs: int = 8
    reconfiguration_freq: Optional[int] = None
    eval_reconfiguration_freq: Optional[int] = 1
    num_steps: int = 100            # matches PlaceSkillEnv max_episode_steps=100
    num_eval_steps: int = 100
    partial_reset: bool = True
    eval_partial_reset: bool = False

    # Grasping is more contact-rich than reaching, so train longer
...
class Agent(nn.Module):
    def __init__(self, envs):
        ...
        # Place obs: qpos(9) + qvel(9) + is_grasped(1) + tcp_pose(7) + obj_pose(7)
        #            + goal_pos(3) + tcp_to_obj_pos(3) + obj_to_goal_pos(3) ≈ 42 dims;
        #            3×256 is adequate capacity.
...
def execute(
    ...
    PLACE_THRESH  = 0.025
    RETREAT_DIST  = 0.10
    REST_Z_THRESH = 0.03   # obstacle half-sizes range 0.015-0.025; +0.01 margin
...
if __name__ == "__main__":
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        clipfracs = []
        update_t  = time.time()
        pg_loss = vf_loss = approx_kl = torch.tensor(0.0, device=device)
